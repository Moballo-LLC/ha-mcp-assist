...
class SearchPackageTool(MCPAssistExternalTool):
    ...
    async def initialize(self) -> None:
        ...
        if provider == "brave":
            from custom_components.mcp_assist.tools.packages.search.brave_search import (
                BraveSearchTool,
            )
            ...
            from custom_components.mcp_assist.tools.packages.search.duckduckgo_search import (
                DuckDuckGoSearchTool,
            )

            self._delegate = DuckDuckGoSearchTool(self.hass)
        elif provider == "searxng":
            from custom_components.mcp_assist.tools.packages.search.searxng_search import (
                SearXNGSearchTool,
            )
