...
def test_gpt6_responses_payload_keeps_reasoning_continuation_and_tools(
    ...
    assert provider.chat_url() == "https://provider.example.invalid/v1/responses"
...
@pytest.mark.parametrize("model_name", ["gpt-6-sol", "gpt-6-luna"])
def test_gpt6_official_chat_completions_tools_use_no_reasoning(model_name: str) -> None:
    provider = OpenAIProvider(
        _settings(
            SERVER_TYPE_OPENAI,
            model_name=model_name,
...
def test_gpt6_custom_chat_adapter_keeps_its_tool_reasoning_contract() -> None:
    provider = OpenAIProvider(
        _settings(SERVER_TYPE_OPENAI, model_name="gpt-6-luna", max_tokens=321)
    )
    payload = provider.build_payload(
        [{"role": "user", "content": "Find a device."}],
        [{"type": "function", "function": {"name": "discover", "parameters": {}}}],
